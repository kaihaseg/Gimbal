...
void GyroInterrupt (void) {
  ...
  int   rawVelocityX       = BMI160.getRotationX();
  float filteredVelocityX  = ExpRunningAverageAdaptive (rawVelocityX);
  float trueVelocityX      = filteredVelocityX / BANANI_V_G_2;

  posX += trueVelocityX * (1 / DEFAULT_GYRO_RATE_IN_HZ) * (1 / DEFAULT_GYRO_RATE_IN_HZ) / 2; 

  int stepperPosToWrite = ComputeStepperPID (posX, initialPosX); 
